...
internal var scytheTowerFireScript: ScytheTowerFireScript;
		...
		if(Time.time >= nextFireTime)
		{
			if (stats.online)
			{
				...
				{		
					...
					scytheTowerFireScript = myProjectile.GetComponent("ScytheTowerFireScript");
					scytheTowerFireScript.level = level;	
					scytheTowerFireScript.flying = flying;	
...
function loadTowerParameters()
{
	...
	stats.hp = excel.scytheTowerArray[level, excel.HP];
	stats.armor = excel.scytheTowerArray[level, excel.ARMOR];
	range = excel.scytheTowerArray[level, excel.RANGE];
	rateOfFire = excel.scytheTowerArray[level, excel.RATE_OF_FIRE];
	cost = excel.scytheTowerArray[level, excel.COST];
	stats.maxHp = stats.hp;
	splashArea = excel.scytheTowerArray[level, excel.ARMOR_PENETRATION];
	healTime = excel.scytheTowerArray[level,excel.HEAL_TIME];
	flying = excel.scytheTowerFlight;
}
...
function OnGUI()
{
	...
	GUI.Label (Rect (healthBarPosition.x, healthBarPosition.y - 20, 100, 20), "Scythe Tower:"+level);
}