...
function OnGUI()
{
	...
	GUI.Label (Rect (healthBarPosition.x, healthBarPosition.y - 20, 100, 20), "Zombie");
}

function loadEnemyParameters()
{
	damage = excel.zombieArray[level, excel.E_DAMAGE];
	hp = excel.zombieArray[level, excel.E_HP];
	armor = excel.zombieArray[level, excel.E_ARMOR];
	range = excel.zombieArray[level, excel.E_RANGE];
	rateOfFire = excel.zombieArray[level, excel.E_RATE_OF_FIRE];
	gain = excel.zombieArray[level, excel.E_GAIN];
	speed = excel.zombieArray[level, excel.E_SPEED];
	mana = excel.zombieArray[level, excel.E_MANA];
	originalSpeed = speed;
}