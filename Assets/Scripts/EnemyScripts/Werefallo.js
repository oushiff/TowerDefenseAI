...
function Start () {
	...
	maxHp = hp;
	numberOfWayPoints = excel.numberOfWayPoints;
	//path traversal
...
function OnGUI()
{
	...
	GUI.Label (Rect (healthBarPosition.x, healthBarPosition.y - 20, 100, 20), "Werefallo");
}

function loadEnemyParameters()
{
	damage = excel.werefalloArray[level, excel.E_DAMAGE];
	hp = excel.werefalloArray[level, excel.E_HP];
	armor = excel.werefalloArray[level, excel.E_ARMOR];
	range = excel.werefalloArray[level, excel.E_RANGE];
	rateOfFire = excel.werefalloArray[level, excel.E_RATE_OF_FIRE];
	gain = excel.werefalloArray[level, excel.E_GAIN];
	speed = excel.werefalloArray[level, excel.E_SPEED];
	mana = excel.werefalloArray[level, excel.E_MANA];
	originalSpeed = speed;
}