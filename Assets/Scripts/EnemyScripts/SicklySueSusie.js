...
function OnTriggerEnter(other: Collider)
{
	...
	else if(other.transform.tag == "ThresherShell")
	{
		...
		print("thresher shell effect:: " + thresherShellEffect);
...
function OnGUI()
{
	...
	GUI.Label (Rect (healthBarPosition.x, healthBarPosition.y - 20, 100, 20), "Sickly Sue Susie");
}

function loadEnemyParameters()
{
	damage = excel.sicklySueArray[level, excel.E_DAMAGE];
	hp = excel.sicklySueArray[level, excel.E_HP];
	armor = excel.sicklySueArray[level, excel.E_ARMOR];
	range = excel.sicklySueArray[level, excel.E_RANGE];
	rateOfFire = excel.sicklySueArray[level, excel.E_RATE_OF_FIRE];
	gain = excel.sicklySueArray[level, excel.E_GAIN];
	speed = excel.sicklySueArray[level, excel.E_SPEED];
	mana = excel.sicklySueArray[level, excel.E_MANA];
	originalSpeed = speed;
}