...
function OnGUI()
{
	...
	GUI.Label (Rect (healthBarPosition.x, healthBarPosition.y - 20, 100, 20), "Frankensteam");
}

function loadEnemyParameters()
{
	damage = excel.frankensteamArray[level, excel.E_DAMAGE];
	hp = excel.frankensteamArray[level, excel.E_HP];
	armor = excel.frankensteamArray[level, excel.E_ARMOR];
	range = excel.frankensteamArray[level, excel.E_RANGE];
	rateOfFire = excel.frankensteamArray[level, excel.E_RATE_OF_FIRE];
	gain = excel.frankensteamArray[level, excel.E_GAIN];
	speed = excel.frankensteamArray[level, excel.E_SPEED];
	mana = excel.frankensteamArray[level, excel.E_MANA];
	originalSpeed = speed;
}