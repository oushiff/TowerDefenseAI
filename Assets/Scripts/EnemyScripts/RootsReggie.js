...
function OnGUI()
{
	...
	GUI.Label (Rect (healthBarPosition.x, healthBarPosition.y - 20, 100, 20), "Roots Reggie");
}

function loadEnemyParameters()
{
	damage = excel.rootReggieArray[level, excel.E_DAMAGE];
	hp = excel.rootReggieArray[level, excel.E_HP];
	armor = excel.rootReggieArray[level, excel.E_ARMOR];
	range = excel.rootReggieArray[level, excel.E_RANGE];
	rateOfFire = excel.rootReggieArray[level, excel.E_RATE_OF_FIRE];
	gain = excel.rootReggieArray[level, excel.E_GAIN];
	speed = excel.rootReggieArray[level, excel.E_SPEED];
	mana = excel.rootReggieArray[level, excel.E_MANA];
	originalSpeed = speed;
}