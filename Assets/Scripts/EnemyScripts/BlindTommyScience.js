...
function OnGUI()
{
	...
	GUI.Label (Rect (healthBarPosition.x, healthBarPosition.y - 20, 100, 20), "Blind Tommy");
	GUI.Label (Rect (healthBarPosition.x, healthBarPosition.y - 40, 100, 20), "HP: " + hp + "/"+ maxHp);
}

function loadEnemyParameters()
{
	damage = excel.blindTommyArray[level, excel.E_DAMAGE];
	hp = excel.blindTommyArray[level, excel.E_HP];
	armor = excel.blindTommyArray[level, excel.E_ARMOR];
	range = excel.blindTommyArray[level, excel.E_RANGE];
	rateOfFire = excel.blindTommyArray[level, excel.E_RATE_OF_FIRE];
	gain = excel.blindTommyArray[level, excel.E_GAIN];
	speed = excel.blindTommyArray[level, excel.E_SPEED];
	mana = excel.blindTommyArray[level, excel.E_MANA];
	originalSpeed = speed;
}